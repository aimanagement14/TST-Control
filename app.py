...
def _migrate_to_roadmap_model(conn):
    ...
    2. Exporta ``profile_graph_nodes`` y ``node_executions`` a un JSON
       (defensivo: las tablas ya no se crean en el esquema actual, pero
       bases de datos legacy pueden tener restos).
    ...
    # Las tablas del editor de grafo + runner se eliminan en init_db()
    # tras llamar a esta migración (DROP redundante eliminado).
...
# Nota: la ruta /settings y los blueprints del editor de grafo (`graph_bp`)
# y del runner (`runner_bp`) no están en esta build. `/settings` se eliminó
# en v2.0 (ADR 2026-09-07); el grafo y el runner quedaron fuera de scope
# del monolito tras la auditoría (ADR 2026-09-11).
